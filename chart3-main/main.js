...
function fixBackgroundSlider() {
    ...
    moveBack.style.transition = 'none';
...
function findClosestAddress() {
    ...
    for (let i = 0; i < buildings.length; i++) {
        ...
        let addy = buildings[i].address[0] + ', ' + buildings[i].address[1];
        let addInput = [];
        ...
        buildings[i].matches = matchesFound;
    }
    let highest = 0;
    let highBuild = false;
    for (let i = 0; i < buildings.length; i++) {
        if (buildings[i].matches > highest) {
            highest = buildings[i].matches;
            highBuild = buildings[i];
        }
    }
...
function showPage3() {
    ...
    drawCanvas(findCuts(building,"BM"),letter.toUpperCase() + 'M');
    $('s3Key').innerHTML = "BM";
    ...
    for (let i = 0; i < boxes.length; i++) {
        ...
        for (let j = 0; j < boxes[i].keys.length; j++) {
            ...
            key.onclick = function() {
                ...
                $('s3Cuts').innerHTML = cuts2;
                drawCanvas(findCuts(building,this.innerHTML),this.innerHTML);
            }
        }
        if (i === 0) {
            let keys = ["TMK","SM","BM"];
...
function drawCanvas(cuts,keyName) {
    let canvas = $('s3Canvas');
    let ctx = canvas.getContext("2d");

    canvas.width = $('s3Title').offsetWidth;
    canvas.height = 200;
    canvas.css({
        width: $('s3Title').offsetWidth + 'px',
        height: "200px",
    })
